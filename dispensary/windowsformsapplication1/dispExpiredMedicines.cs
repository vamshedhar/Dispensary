...
{
    public partial class dispExpiredMedicines : Form
    {
        string username;
        ...
        public dispExpiredMedicines(string user)
        {
            InitializeComponent();
            username = user;
            loadMedicines();
        ...
        private void dispExpiredMedicines_Load(object sender, EventArgs e)
        {
            dataGridView1.ClearSelection();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox6.Text != "")
            {
                if (textBox4.Text != "")
                {
                    if (textBox13.Text != "")
                    {
                        int dispStock = Convert.ToInt32(textBox11.Text);
                        int quantity = Convert.ToInt32(textBox13.Text);
                        if (dispStock - quantity >= 0)
                        {
                            string receiver = "miscellaneous";
                            store st = new store();
                            st.dispExpiredStock(dispStock, quantity, textBox6.Text, textBox4.Text, username, receiver, textBox7.Text);
                            loadMedicines();
                            textBox5.Text = "";
                            clearData();
                        }
