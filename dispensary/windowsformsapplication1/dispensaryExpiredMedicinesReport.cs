...
{
    public partial class dispensaryExpiredMedicinesReport : Form
    {
        string username;
        int j = 0;
        public dispensaryExpiredMedicinesReport(string user)
        {
            username = user;
            InitializeComponent();
        }
        ...
        {
            store med = new store();
            med.loadExpiredMedicine("dispensary");
        ...
        private void dispensaryExpiredMedicinesReport_Load(object sender, EventArgs e)
        {
            yearsLoad();
        ...
        private void printDocument1_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            store.expiredStockReport("dispensary", monthCalendar1.SelectionRange.Start.ToString("yyyyMM") + "01", monthCalendar1.SelectionRange.Start.AddMonths(1).ToString("yyyyMM") + "01", db);
            int i = 1;
            while (store.dr.Read())
            {
                if (i < j)
        ...
        private void printDocument2_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            dbconnect db = new dbconnect("medicines");
            //MessageBox.Show("SELECT medicine_name, medicine_type, quantity, timestamp, user FROM expired WHERE transaction_id>='" + monthCalendar1.SelectionRange.Start.ToString("yyyyMM") + "01" + "' AND transaction_id<'" + monthCalendar1.SelectionRange.Start.AddMonths(1).ToString("yyyyMM") + "01" + "'");
            store.expiredStockReport("dispensary", comboBox1.SelectedItem.ToString() + "0401", (Convert.ToInt32(comboBox1.SelectedItem.ToString()) + 1).ToString() + "0401", db);
            int i = 1;
            while (store.dr.Read())
            {
                if (i < j)
